...
import { 
  User, Calendar, Eye, MessageSquare, ArrowLeft, 
  Share2, Facebook, Twitter, Link, Check, Send, Sparkles 
...
const mockPosts: BlogPost[] = [
  ...
  },
  {
    ...
    summary: 'Nếu chỉ có ngân sách dưới 1 triệu đồng nhưng vẫn muốn có 1 chiếc tai nghe chống ồn "xịn", pin khủng và nhiều tính năng từ 1 thương hiệu đáng tin cậy thì soundcore R60i NC là 1 lựa chọn rất đáng cân nhắc. Đặc biệt, nếu so với mẫu R50i NC, chiếc tai nghe này có sự nâng cấp rất mạnh mẽ.',
  ...
  },
  {
    ...
    summary: 'Sau khi công bố kết quả kinh doanh quý I năm tài chính 2026 với nhiều tín hiệu tích cực, CEO Apple Tim Cook đã tham gia buổi họp với giới phân tích và chia sẻ thêm về mức độ đón nhận của Apple Intelligence trên iPhone. Theo ông, tính năng được người dùng sử dụng nhiều nhất hiện nay không phải Siri, mà là Visual Intelligence - một công cụ trí tuệ nhân tạo dựa trên camera, có cách dùng khá tương đồng với Google Lens.',
    slug: 'tim-cook-he-lo-tinh-nang-apple-intelligence-duoc-yeu-thich'
  },
  {
    id: '4',
    title: '(iOS/Android) Nhận key gói Premium ứng dụng giúp thư giãn, ngủ ngon, chữa bệnh trị giá $399.99',
    category: 'Mobile',
    author: 'Nguyễn Cường',
    publishDate: 'Hôm nay lúc 20:31',
    views: 9805,
    comments: 5,
    imageUrl: 'https://images.unsplash.com/photo-1511295742364-92b9345f8e00?auto=format&fit=crop&w=800&q=80',
    summary: 'Nếu bạn đang tìm kiếm một giải pháp để thư giãn tâm trí, ngủ ngon hơn, giảm căng thẳng và lấy lại sự cân bằng trong cuộc sống, thì Calm chính là ứng dụng không thể bỏ qua. Calm hiện đang là ứng dụng số 1 thế giới về thiền, giấc ngủ và sức khỏe tinh thần, được hàng triệu người tin dùng mỗi ngày để tìm lại sự bình yên bên trong mình.',
  ...
  },
  {
    ...
    summary: 'Công nghệ thực tế ảo và thực tế hỗn hợp đang tiến gần hơn tới đời sống thường nhật với hàng loạt kính thông minh tích hợp trí tuệ nhân tạo ra đời trong năm 2026.',
    slug: 'cuoc-cach-mang-thuc-te-ao-tiep-theo'
  },
  {
    id: 'gs1',
    title: 'VCS Mùa Hè 2026 chính thức khởi tranh: Cuộc chiến khốc liệt giành vé đi Chung Kết Thế Giới',
    category: 'Esports',
    author: 'LeagueFan',
    publishDate: '28 Tháng 5, 2026',
    views: 5800,
  ...
  },
  {
    ...
    summary: 'Sự kiện Marvel crossover mới nhất đã tạo nên cơn sốt lịch sử, kéo hàng triệu người chơi đăng nhập cùng một lúc.',
    slug: 'fortnite-dat-ky-luc-nguoi-choi-moi'
  },
  {
    id: 'gs3',
    title: 'GTA 6 lộ diện trailer tiếp theo: Thành phố Vice City chưa bao giờ sống động và chân thực đến thế',
    category: 'RPG',
    author: 'RockstarFan',
    publishDate: '27 Tháng 5, 2026',
    views: 10450,
    comments: 54,
    imageUrl: 'https://images.unsplash.com/photo-1612287230202-1bf1d85d1bdf?auto=format&fit=crop&w=400&q=80',
    summary: 'Rockstar Games vừa làm rúng động cộng đồng game thủ thế giới khi tung ra đoạn trailer thứ 2 cho bom tấn Grand Theft Auto VI (GTA 6).',
  ...
  },
  {
    ...
    views: 4500,
    comments: 12,
    imageUrl: 'https://images.unsplash.com/photo-1595169001925-51e18ede8436?auto=format&fit=crop&w=400&q=80',
    summary: 'Tin đồn mới nhất cho thấy thiết bị chơi game thế hệ tiếp theo của Nintendo sẽ chính thức ra mắt cấu hình chi tiết cuối năm nay.',
...
const post = computed<BlogPost>(() => {
  ...
  return {
    id: 'fallback',
    title: slug.value
      ? slug.value.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')
    ...
    summary: 'Thông tin chi tiết về sản phẩm công nghệ đang thu hút sự chú ý của giới công nghệ trong và ngoài nước.',
...
const comments = ref<Comment[]>([
  {
    ...
    date: '3 giờ trước',
    content: 'Bài viết phân tích rất chi tiết và khách quan. Mình cũng đang tính gom lúa để lên đời sản phẩm này, hy vọng sẽ đúng như kỳ vọng.',
    likes: 8
  },
  {
    id: 2,
    ...
    date: '1 giờ trước',
    content: 'Thiết kế đẹp thực sự, nhìn rất cao cấp. Mong là bản thương mại chính thức tại Việt Nam sẽ giữ nguyên được cấu hình và các ưu đãi quà tặng.',
    likes: 3
  }
])
...
<template>
  <div class="min-h-screen bg-gray-50 dark:bg-zinc-950 text-zinc-800 dark:text-zinc-200 transition-colors duration-300 font-sans">
    ...
    <header class="bg-white dark:bg-zinc-900 border-b border-gray-250 dark:border-zinc-800 sticky top-0 z-50 shadow-sm">
      <div class="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <NuxtLink to="/" class="flex items-center gap-2 hover:opacity-90 transition-opacity">
          ...
          <span class="hidden sm:inline-block px-2 py-0.5 text-[10px] font-bold bg-[#f39c12] text-white rounded">TECH</span>
        </NuxtLink>
        ...
        <div class="flex items-center gap-4">
          <button class="text-zinc-550 dark:text-zinc-400 hover:text-zinc-800 dark:hover:text-white transition-colors cursor-pointer">
    ...
    <!-- Breadcrumbs -->
    <div class="bg-gray-100 dark:bg-zinc-900 border-b border-gray-200 dark:border-zinc-850 py-3 text-xs">
      <div class="container mx-auto px-4 flex items-center gap-2 text-zinc-500">
        ...
        <span>/</span>
        <span class="text-zinc-700 dark:text-zinc-300 font-medium truncate max-w-[200px] sm:max-w-xs md:max-w-md">
          {{ post.title }}
        </span>
      </div>
    </div>
    ...
    <div class="container mx-auto px-4 py-8">
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <article class="lg:col-span-8 space-y-6">
          ...
          <!-- Category and Meta -->
          <div class="space-y-4">
            <span class="inline-block text-[10px] font-extrabold uppercase tracking-widest px-2.5 py-1 rounded bg-[#3498db]/10 text-[#3498db] dark:bg-[#e74c3c]/10 dark:text-[#e74c3c]">
              {{ post.category }}
            </span>
            <h1 class="text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight text-zinc-900 dark:text-white leading-tight">
              {{ post.title }}
            </h1>

            <!-- Meta statistics -->
            <div class="flex flex-wrap items-center gap-y-2 gap-x-4 text-xs text-zinc-500 border-b border-gray-200 dark:border-zinc-850 pb-4">
              <span class="flex items-center gap-1.5">
                <User class="w-4 h-4" />
                Đăng bởi <strong class="text-zinc-700 dark:text-zinc-300 font-semibold">{{ post.author }}</strong>
              </span>
              <span class="flex items-center gap-1.5">
          ...
          <!-- Feature Image -->
          <div class="relative rounded-2xl overflow-hidden aspect-video shadow-md border border-gray-100 dark:border-zinc-900">
            <img 
              :src="post.imageUrl" 
              :alt="post.title"
          ...
          <!-- Post Content Body -->
          <div class="prose prose-zinc dark:prose-invert max-w-none text-zinc-650 dark:text-zinc-350 text-sm leading-relaxed space-y-6">
            <p class="text-base font-semibold text-zinc-900 dark:text-white leading-relaxed first-letter:text-4xl first-letter:font-black first-letter:float-left first-letter:mr-2">
              {{ postContent.intro }}
            </p>
            ...
            <!-- Pull Quote Block -->
            <blockquote class="border-l-4 border-[#3498db] dark:border-[#e74c3c] bg-gray-100 dark:bg-zinc-900 p-5 rounded-r-xl my-6 italic text-zinc-700 dark:text-zinc-300 font-medium">
              "{{ postContent.quote }}"
            </blockquote>
          ...
          <!-- Sharing Actions -->
          <div class="flex flex-wrap items-center justify-between border-y border-gray-200 dark:border-zinc-850 py-4 gap-4">
            <span class="text-xs font-bold flex items-center gap-1.5 text-zinc-700 dark:text-zinc-300">
          ...
          <div class="space-y-6 pt-6">
            <h3 class="text-lg font-black uppercase text-zinc-900 dark:text-white tracking-tight flex items-center gap-2">
              <MessageSquare class="w-5 h-5 text-[#3498db] dark:text-[#e74c3c]" />
              Bình luận ({{ comments.length }})
            </h3>

            <!-- Comment Form -->
            <form @submit.prevent="handleAddComment" class="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-gray-250 dark:border-zinc-850 shadow-xs space-y-4">
            ...
            <div class="space-y-4">
              ...
              >
                ...
                <div class="flex-grow space-y-1">
                  ...
                  <p class="text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">{{ c.content }}</p>
        ...
        <aside class="lg:col-span-4 space-y-8">
          <!-- Popular news widget -->
          <div class="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-gray-200 dark:border-zinc-850 shadow-xs">
            <h4 class="text-sm font-black uppercase text-zinc-900 dark:text-white border-b border-gray-200 dark:border-zinc-850 pb-3 mb-4 tracking-tight flex items-center gap-2">
              🔥 Xem nhiều nhất
            </h4>
            <div class="space-y-4">
              <div 
              ...
              >
                <span class="text-xl font-black text-zinc-300 dark:text-zinc-700 italic shrink-0 w-6 text-center">
                  {{ index + 1 }}
                </span>
                <div class="space-y-1">
                  ...
                  <h5 class="text-xs font-bold leading-snug text-zinc-900 dark:text-white hover:text-[#3498db] dark:hover:text-[#e74c3c] transition-colors line-clamp-2">
          ...
          <!-- Newsletter Banner -->
          <div class="bg-gradient-to-tr from-sky-600 to-indigo-900 dark:from-red-950 dark:to-zinc-900 p-6 rounded-2xl text-white space-y-4 shadow-md relative overflow-hidden">
            <div class="absolute -top-10 -left-10 w-32 h-32 rounded-full bg-white/5 blur-2xl"></div>
            <h4 class="text-base font-black uppercase tracking-tight relative z-10">Bản tin TechDeal</h4>
            <p class="text-xs text-zinc-200 leading-relaxed relative z-10">
              Đăng ký nhận thông tin công nghệ, game và khuyến mãi sớm nhất từ tòa soạn của chúng tôi.
            </p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <div class="p-5 flex-grow flex flex-col justify-between">
              <div>
                <span class="text-[9px] font-extrabold uppercase tracking-wider text-[#3498db] mb-2 inline-block">
                  {{ rp.category }}
                </span>
                <h4 class="text-xs sm:text-sm font-bold text-zinc-900 dark:text-white group-hover:text-[#3498db] dark:group-hover:text-[#e74c3c] transition-colors leading-snug line-clamp-2">
                  <NuxtLink :to="`/blog/${rp.slug}.${rp.id}`">{{ rp.title }}</NuxtLink>
                </h4>
              </div>
              <div class="flex items-center justify-between text-[10px] text-zinc-500 pt-3 mt-4 border-t border-gray-100 dark:border-zinc-850/50">
