      ...
      if (g) {
        ...
        if (googleBtnEl) {
          g.accounts.id.renderButton(googleBtnEl, {
            theme: 'outline',
            size: 'large',
            width: googleBtnEl.clientWidth || 380,
            text: 'signin_with',
            shape: 'rectangular'
          })
        }
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12">
      ...
      >
        ...
        <div class="relative w-full overflow-hidden rounded-xl">
          <button
            type="button"
            class="w-full py-3 border border-gray-200 dark:border-zinc-800 rounded-xl bg-white dark:bg-zinc-955 text-zinc-700 dark:text-zinc-300 text-xs font-bold flex items-center justify-center gap-2 hover:bg-gray-50 dark:hover:bg-zinc-900 transition-colors shadow-xs"
          >
            <!-- SVG Google Icon -->
          ...
          <!-- Hidden Google Native Button that captures clicks -->
          <div
            id="google-btn-container"
            class="absolute inset-0 opacity-0 cursor-pointer overflow-hidden z-10 [&_iframe]:!w-full [&_iframe]:!h-full [&_iframe]:!max-w-none"
          ></div>
        </div>
